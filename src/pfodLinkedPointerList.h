...
*/
template<typename T>
bool pfodLinkedPointerList<T>::append(T* _t) {
  if (_t == NULL) {
    return false;
  }
  size_t idx = getIndex(_t);
  ...
  if (root == NULL) {
    ...
    pfodPointerListNode<T> *currentPtr = root;
...
*/
template<typename T>
size_t pfodLinkedPointerList<T>::getIndex(T * _t) {
  if ((root == NULL) || (_t == NULL)) {
    return size();
  }
  size_t idx = 0;
  pfodPointerListNode<T>* listPtr = root;
  while (listPtr) {
...
*/
template<typename T>
T* pfodLinkedPointerList<T>::getCurrentIterator() {
  if (current) {
    return current->data;
  } //
  return NULL;
...
*/
template<typename T>
bool pfodLinkedPointerList<T>::setCurrentIterator(T* _current) {
  current = NULL;
...
*/
template<typename T>
T* pfodLinkedPointerList<T>::remove(size_t idx) {
...
*/
template<typename T>
bool pfodLinkedPointerList<T>::insertAt(T* _t, size_t idx) {
  ...
  pfodPointerListNode<T>* currentPtr = root;
  size_t counter = 0;
  while ((counter < idx_1) && (currentPtr != NULL)) {
    currentPtr = currentPtr->next;
    counter++;
  }
  // currentPtr is idx-1
  pfodPointerListNode<T>* lastListPtr = currentPtr;
  pfodPointerListNode<T>* nextListPtr = currentPtr->next;
  // insert
...
*/
template<typename T>
T* pfodLinkedPointerList<T>::get(size_t idx) {
  pfodPointerListNode<T>* currentPtr = root;
  if (currentPtr == NULL) {
    return NULL;
  } // else
  size_t counter = 0;
  while ((counter < idx) && (currentPtr != NULL)) {
    currentPtr = currentPtr->next;
    counter++;
  }
  if (currentPtr) {
